...
We show that it is necessary at least for the equality, which is an indexed inductive proposition:
 *)
...
*)
  Lemma eqUnif (A1 A2:Set) (AR : A1 -> A2 -> Prop) (* some assumption about AR *) :
    forall   (l1: A1) (l2: A2) (lR : AR l1 l2)
...
copying what we had to prove for the uniformity of the equality type. 

What is surprising is that it sufficies even for other indexed inductive propositions, 
and that we can compositionally build the proof 
...
(** our current definition of one to one is slightly different but logically equivalent. *)
  Definition OneToOne {A B : Set} (R : A → B → Prop) : Prop
    := (∀ (a:A) (b1 b2: B), R a b1 → R a b2 → b1=b2)
       ∧ (∀ (b:B) (a1 a2: A), R a1 b → R a2 b → a1=a2).
