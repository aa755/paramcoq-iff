...
fix
add_R (n n₂ : nat) (n_R : Coq_Init_Datatypes_nat_pmtcty_RR0 n n₂) 
      (m m₂ : nat) (m_R : Coq_Init_Datatypes_nat_pmtcty_RR0 m m₂) {struct n} :
  Coq_Init_Datatypes_nat_pmtcty_RR0 (add n m) (add₂ n₂ m₂) :=
  @UsefulTypes.transport nat match n return nat with
                             | O => m
                             | S p => S (add p m)
                             end (add n m)
    (fun equ : nat => Coq_Init_Datatypes_nat_pmtcty_RR0 equ (add₂ n₂ m₂))
    ((fun n0 m0 : nat =>
      match
        n0 as n1
        return
          (@eq nat match n1 return nat with
                   | O => m0
                   | S p => S (add p m0)
                   end (add n1 m0))
      with
      | O => @eq_refl nat match O return nat with
                          | O => m0
                          | S p => S (add p m0)
                          end
      | S x => @eq_refl nat match S x return nat with
                            | O => m0
                            | S p => S (add p m0)
                            end
      end) n m)
    (@UsefulTypes.transport nat
       match n₂ return nat with
       | O => m₂
       | S p₂ => S (add₂ p₂ m₂)
       end (add₂ n₂ m₂)
       ...
       ((fun n₂0 m₂0 : nat =>
         match
           n₂0 as n₂1
           return
             (@eq nat match n₂1 return nat with
                      | O => m₂0
                      | S p₂ => S (add₂ p₂ m₂0)
                      end (add₂ n₂1 m₂0))
         with
         | O =>
             @eq_refl nat match O return nat with
                          | O => m₂0
                          | S p₂ => S (add₂ p₂ m₂0)
                          end
         | S x =>
             @eq_refl nat match S x return nat with
                          | O => m₂0
                          | S p₂ => S (add₂ p₂ m₂0)
                          end
         end) n₂ m₂)
       (match
          ...
          return
            ((fun n1 n₂0 : nat : Set =>
              forall _ : Coq_Init_Datatypes_nat_pmtcty_RR0 n1 n₂0,
              Coq_Init_Datatypes_nat_pmtcty_RR0
        ...
        | O =>
            match
              ...
              return
                ((fun n0 n₂1 : nat : Set =>
                  forall _ : Coq_Init_Datatypes_nat_pmtcty_RR0 n0 n₂1,
                  Coq_Init_Datatypes_nat_pmtcty_RR0
                    match n0 return nat with
                    | O => m
                    | S p => S (add p m)
                    end match n₂1 return nat with
                        | O => m₂
                        | S p₂ => S (add₂ p₂ m₂)
                        end) O n₂0)
            with
            | O =>
                fun n_R0 : Coq_Init_Datatypes_nat_pmtcty_RR0 O O =>
                Coq_Init_Datatypes_nat_pmtcty_RR0_constr_0_inv n_R0
                  (fun _ : Coq_Init_Datatypes_nat_pmtcty_RR0 O O =>
                   Coq_Init_Datatypes_nat_pmtcty_RR0
                     ...
                     | O => m
                     | S p => S (add p m)
                     end match O return nat with
                         | O => m₂
                         | S p₂ => S (add₂ p₂ m₂)
                         end) m_R
            | S p₂ =>
                fun n_R0 : Coq_Init_Datatypes_nat_pmtcty_RR0 O (S p₂) =>
                False_rectt
                  (Coq_Init_Datatypes_nat_pmtcty_RR0
                     match O return nat with
                     | O => m
                     | S p => S (add p m)
                     end match S p₂ return nat with
                         | O => m₂
                         | S p₂0 => S (add₂ p₂0 m₂)
                         end) n_R0
            end
        | S p =>
            ...
            with
            | O =>
                fun n_R0 : Coq_Init_Datatypes_nat_pmtcty_RR0 (S p) O =>
                False_rectt
                  (Coq_Init_Datatypes_nat_pmtcty_RR0
                     match S p return nat with
                     | O => m
                     | S p0 => S (add p0 m)
                     end match O return nat with
                         | O => m₂
